...
for (const marker of ['SYNAPSEMAX_FINANCE_PRODUCTIZATION_V2','impact-error-recovery','impact-delay-recovery','impact-error-overlap','impact-delay-overlap','impact-error-delay-overlap','impact-evidence','impact-revenue','impact-margin','impact-capex','impact-opex','impact-annual-opex','finance-impact-button','finance-impact-scenarios','data-scenario=','Action Map','marginUpliftPoints','recoverableErrorShare','recoverableDelayShare','oneTimeCapex','monthlyOpex','annualOpex']) {
  if (!html.includes(marker)) throw new Error('Finance productization test: missing ' + marker);
}
if (!html.includes("['conservative','base','optimistic']")) throw new Error('Finance productization test: scenario set missing');
