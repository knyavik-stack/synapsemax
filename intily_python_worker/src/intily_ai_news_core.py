...
async def get(url, timeout=12, headers=None):
    response = await fetch(url, headers=headers or {'User-Agent': 'IntilyAI-News/7.0'})
    if not response.ok:
        raw = await response.text()
        raise RuntimeError(f'HTTP_{int(response.status)}: {raw[:300]}')
...
async def chat(url, model, token, prompt, provider, retries=2):
    body = json.dumps({
        'model': model,
        'messages': [
    ...
    for attempt in range(retries):
        try:
            ...
            if not response.ok:
                ...
                last = RuntimeError(f'{provider}_HTTP_{code}: {raw[:300]}')
                if provider == 'OPENAI' and code == 429: raise last
                if provider == 'GROQ' and code == 403: raise last
                if code not in (429,500,502,503,504): raise last
                retry_after = response.headers.get('Retry-After')
                wait = min(int(retry_after),30) if retry_after and str(retry_after).isdigit() else min(2 ** attempt * 3,20)
                print(provider + '_RETRY', code, wait)
            ...
            if attempt < retries-1:
                ...
                print(provider + '_RETRY_EXCEPTION', wait)
                await asyncio.sleep(wait)
...
async def gemini_chat(prompt, token):
    body=json.dumps({
    ...
    for attempt in range(3):
        try:
            response=await fetch(GEMINI_URL + '?key=' + urllib.parse.quote(token,safe=''), method='POST', headers={'Content-Type':'application/json'}, body=body)
            raw=await response.text()
            if not response.ok:
                code=int(response.status); last=RuntimeError(f'GEMINI_HTTP_{code}: {raw[:300]}')
                if code not in (500,502,503,504,429): raise last
                wait=min(5*(2**attempt),20); print('GEMINI_RETRY',code,wait); await asyncio.sleep(wait); continue
            d=json.loads(raw)
            ...
            last=e
            if attempt<2:
                wait=min(5*(2**attempt),20); print('GEMINI_RETRY_EXCEPTION',wait); await asyncio.sleep(wait)
    raise last or RuntimeError('GEMINI_FAILED')
...
async def ai(prompt, s, ai_binding=None):
    errors = []
    if ai_binding is not None:
        try:
            result = await ai_binding.run(CLOUDFLARE_AI_MODEL, {
                'messages': [
                    {'role': 'system', 'content': 'Ты профессиональный редактор русского Telegram-канала об AI. Отвечай только валидным JSON.'},
                    {'role': 'user', 'content': prompt}
                ],
            ...
            text = result.get('response') if hasattr(result, 'get') else getattr(result, 'response', None)
            if text and len(str(text).strip()) > 20:
                print('AI_PROVIDER_OK', 'CLOUDFLARE_AI')
                return str(text)
            errors.append('CLOUDFLARE_AI: EMPTY_RESPONSE')
        except Exception as ex:
            errors.append('CLOUDFLARE_AI: ' + str(ex)[:180])
            print('AI_PROVIDER_FAILED', 'CLOUDFLARE_AI', str(ex)[:180])
    ...
    for name, url, model, token in providers:
        ...
        try:
            ...
            if name == 'GEMINI':
                result = await gemini_chat(prompt, token)

            else:
                result = await chat(url, model, token, prompt, name)
...
async def edit(x, s, ai_binding=None):
    last_error = ''
    ...
    for attempt in range(
        ...
        try:
            ...
            raw = await ai(prompt, s, ai_binding)
...
async def telegram(text):
    ...
    payload=json.dumps({'chat_id':chat_id,'text':text,'parse_mode':'HTML','disable_web_page_preview':True})
    last=None
    for attempt in range(3):
        try:
            response=await fetch(TG_URL.format(token),method='POST',headers={'Content-Type':'application/json'},body=payload)
            raw=await response.text()
            d=json.loads(raw)
            if response.ok and d.get('ok'):
                print('TELEGRAM_SENT',d.get('result',{}).get('message_id')); return
            last=RuntimeError(raw)
            retry_after=d.get('parameters',{}).get('retry_after')
            wait=min(int(retry_after),30) if retry_after else min(2**attempt*2,15)
            print('TELEGRAM_RETRY',int(response.status),wait); await asyncio.sleep(wait)
        except Exception as e:
            last=e; wait=min(2**attempt*2,10); print('TELEGRAM_RETRY_EXCEPTION',wait); await asyncio.sleep(wait)
...
async def main(load_state_fn=None, save_state_fn=None, ai_binding=None):
    s = await load_state_fn() if load_state_fn else load_state()
    now = time.time()
    cut = now - 30 * 86400
    health = s['health']
    ...
    if not can_publish:
        ...
        for x in publish_candidates:
            ...
            attempts += 1
            try:
                post = await edit(x, s, ai_binding)
