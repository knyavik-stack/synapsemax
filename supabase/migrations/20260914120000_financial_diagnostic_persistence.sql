-- SynapseMax minimal persistence contract
-- IMPORTANT: do not apply to an unrelated Supabase project.
-- This migration is intentionally not wired to deployment yet.
...
-- Server-side tenant context. The application must set this transaction-locally
-- only after authenticating the principal and resolving membership.
...
as $$
begin
  raise exception 'APPEND_ONLY_RESOURCE: % cannot be updated or deleted', TG_TABLE_NAME
    using errcode = '55000';
end;
$$;

-- Calculation results and audit events are immutable by design.
...
before update or delete on public.calculation_results
...
create trigger input_snapshots_append_only
before update or delete on public.diagnostic_input_snapshots
for each row execute function public.enforce_append_only();

drop trigger if exists result_lineage_append_only on public.calculation_result_lineage;
create trigger result_lineage_append_only
before update or delete on public.calculation_result_lineage
for each row execute function public.enforce_append_only();

-- Evidence is corrected through a new revision. Existing evidence rows are immutable.
...
-- Tenant-scoped RLS. Do not expose these tables to a client role until the
-- application identity path is implemented and grants are explicitly reviewed.
...
-- The policies below are intentionally limited to the transaction tenant context.
-- The final production migration must also define exact role grants after the
-- application authentication strategy is selected.

create policy tenants_isolation on public.tenants
  for all using (id = public.current_tenant_id())
  with check (id = public.current_tenant_id());

create policy memberships_isolation on public.tenant_memberships
  for all using (tenant_id = public.current_tenant_id())
  with check (tenant_id = public.current_tenant_id());

create policy legal_holds_isolation on public.tenant_legal_holds
  for all using (tenant_id = public.current_tenant_id())
  with check (tenant_id = public.current_tenant_id());

create policy diagnostic_sessions_isolation on public.diagnostic_sessions
  for all using (tenant_id = public.current_tenant_id())
  with check (tenant_id = public.current_tenant_id());

create policy evidence_items_isolation on public.evidence_items
  for all using (tenant_id = public.current_tenant_id())
  with check (tenant_id = public.current_tenant_id());

create policy input_snapshots_isolation on public.diagnostic_input_snapshots
  for all using (tenant_id = public.current_tenant_id())
  with check (tenant_id = public.current_tenant_id());

create policy calculation_results_isolation on public.calculation_results
  for all using (tenant_id = public.current_tenant_id())
  with check (tenant_id = public.current_tenant_id());

create policy result_lineage_isolation on public.calculation_result_lineage
  for all using (tenant_id = public.current_tenant_id())
  with check (tenant_id = public.current_tenant_id());
...
-- No implicit Data API exposure is granted here. Explicit grants belong in the
-- deployment-specific migration after the authenticated application path exists.
revoke all on table
  public.tenants,
