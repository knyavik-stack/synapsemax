...
create table if not exists public.retention_policies (
  ...
  policy jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(),
...
create table if not exists public.tenants (
  ...
  retention_policy_id uuid references public.retention_policies(id), created_at timestamptz not null default now(),
  constraint tenants_status_check check (status in ('active', 'suspended', 'archived'))
);
create table if not exists public.tenant_memberships (
  tenant_id uuid not null references public.tenants(id) on delete cascade, principal_id text not null,
...
create table if not exists public.tenant_legal_holds (
  ...
  scope text not null default 'tenant', reason_code text not null, active boolean not null default true,
...
create table if not exists public.diagnostic_sessions (
  ...
  status text not null default 'created', created_by text not null, calculation_contract_version text not null,
  input_snapshot_id uuid, evidence_set_id text, created_at timestamptz not null default now(),
  constraint diagnostic_sessions_status_check check (status in ('created', 'calculating', 'completed', 'failed', 'archived')),
  unique (tenant_id, id)
);
create table if not exists public.evidence_items (
  id uuid primary key default gen_random_uuid(), tenant_id uuid not null references public.tenants(id),
  supersedes_evidence_id uuid, source_type text not null, source_ref text not null,
  observed_at timestamptz not null, collected_at timestamptz not null, metric text not null,
  value jsonb not null, unit text not null, quality text not null, provenance_hash text not null,
  ...
  constraint evidence_items_metadata_check check (jsonb_typeof(metadata) = 'object'), unique (tenant_id, id)
);
DO $$ BEGIN
  IF NOT EXISTS (select 1 from pg_constraint where conname = 'evidence_items_supersedes_same_tenant_fk') THEN
    alter table public.evidence_items add constraint evidence_items_supersedes_same_tenant_fk
      foreign key (tenant_id, supersedes_evidence_id) references public.evidence_items(tenant_id, id);
  END IF;
END $$;

create table if not exists public.diagnostic_input_snapshots (
  id uuid primary key default gen_random_uuid(), tenant_id uuid not null references public.tenants(id),
  session_id uuid not null, schema_version text not null, payload_hash text not null,
  normalized_payload jsonb not null, created_at timestamptz not null default now(),
  constraint diagnostic_input_snapshots_payload_check check (jsonb_typeof(normalized_payload) = 'object'),
  unique (tenant_id, id), foreign key (tenant_id, session_id) references public.diagnostic_sessions(tenant_id, id)
);
create table if not exists public.calculation_results (
  id uuid primary key default gen_random_uuid(), tenant_id uuid not null references public.tenants(id),
  session_id uuid not null, calculation_contract_version text not null, scenario text not null,
  gross_recoverable_monthly numeric(20,2) not null, overlap_adjustment_monthly numeric(20,2) not null default 0,
  net_recoverable_monthly numeric(20,2) not null, annual_net_value numeric(20,2) not null,
  upfront_investment numeric(20,2) not null default 0, annual_opex numeric(20,2) not null default 0,
  roi_percent numeric(20,6), payback_months numeric(20,6), margin_uplift_points numeric(20,6),
  evidence_quality text not null, assumptions jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(),
...
create table if not exists public.calculation_result_lineage (
  result_id uuid not null, tenant_id uuid not null references public.tenants(id), evidence_item_id uuid,
  input_snapshot_id uuid not null, scenario text not null, created_at timestamptz not null default now(),
...
create table if not exists public.audit_events (
  id uuid primary key default gen_random_uuid(), tenant_id uuid references public.tenants(id), actor_type text not null,
  actor_id text not null, action text not null, resource_type text not null, resource_id text not null,
  occurred_at timestamptz not null default now(), request_id text not null, result text not null,
  reason_code text, metadata jsonb not null default '{}'::jsonb,
...
create table if not exists public.idempotency_keys (
  tenant_id uuid not null references public.tenants(id) on delete cascade, request_id text not null,
  operation text not null, resource_id uuid, response_hash text, created_at timestamptz not null default now(),
...
$$;
create or replace function public.enforce_append_only() returns trigger language plpgsql as $$
begin
...
DO $$ BEGIN
  ...
  drop policy if exists memberships_isolation on public.tenant_memberships;
  drop policy if exists legal_holds_isolation on public.tenant_legal_holds;
  drop policy if exists diagnostic_sessions_isolation on public.diagnostic_sessions;
  drop policy if exists evidence_items_isolation on public.evidence_items;
  drop policy if exists input_snapshots_isolation on public.diagnostic_input_snapshots;
  drop policy if exists calculation_results_isolation on public.calculation_results;
  drop policy if exists result_lineage_isolation on public.calculation_result_lineage;
...
END $$;
create policy tenants_isolation on public.tenants for all using (id = public.current_tenant_id()) with check (id = public.current_tenant_id());
create policy memberships_isolation on public.tenant_memberships for all using (tenant_id = public.current_tenant_id()) with check (tenant_id = public.current_tenant_id());
create policy legal_holds_isolation on public.tenant_legal_holds for all using (tenant_id = public.current_tenant_id()) with check (tenant_id = public.current_tenant_id());
create policy diagnostic_sessions_isolation on public.diagnostic_sessions for all using (tenant_id = public.current_tenant_id()) with check (tenant_id = public.current_tenant_id());
create policy evidence_items_isolation on public.evidence_items for all using (tenant_id = public.current_tenant_id()) with check (tenant_id = public.current_tenant_id());
create policy input_snapshots_isolation on public.diagnostic_input_snapshots for all using (tenant_id = public.current_tenant_id()) with check (tenant_id = public.current_tenant_id());
create policy calculation_results_isolation on public.calculation_results for all using (tenant_id = public.current_tenant_id()) with check (tenant_id = public.current_tenant_id());
create policy result_lineage_isolation on public.calculation_result_lineage for all using (tenant_id = public.current_tenant_id()) with check (tenant_id = public.current_tenant_id());
create policy audit_events_isolation on public.audit_events for all using (tenant_id = public.current_tenant_id()) with check (tenant_id = public.current_tenant_id());
create policy idempotency_keys_isolation on public.idempotency_keys for all using (tenant_id = public.current_tenant_id()) with check (tenant_id = public.current_tenant_id());

revoke all on function public.current_tenant_id() from public;
revoke all on function public.enforce_append_only() from public;
comment on function public.current_tenant_id() is 'Reads transaction-local app.tenant_id. Application must derive tenant from authenticated principal; client input is never authoritative.';
comment on table public.calculation_results is 'Immutable financial result history. Updates/deletes are rejected by trigger.';
comment on table public.audit_events is 'Tenant-scoped immutable audit history. Tenantless system events require a separate privileged server path and are not visible to tenant context.';
