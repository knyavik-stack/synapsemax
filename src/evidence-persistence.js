...
async function apiRequest(path, { method = 'POST', body, authorization, prefer = 'return=representation' } = {}) {
  ...
  if (!response.ok) throw new Error('Data API request failed (' + response.status + '): ' + text.slice(0, 500));
  return data;
}
...
export async function persistEvidenceDiagnostic({ request, tenantContext, input }) {
  ...
  if (!Array.isArray(idem) || idem.length === 0) {
    ...
    return { idempotentReplay: true, sessionId: row.resource_id, requestId, payloadHash: row.response_hash };
  }
  ...
  try {
    ...
    await apiRequest('/diagnostic_sessions?id=eq.' + encodeURIComponent(sessionId), {
      method: 'PATCH',
    ...
    try {
      ...
      await apiWrite('/audit_events', {
        ...
        actor_type: 'principal',
        actor_id: tenantContext.principalId ?? 'authenticated-principal',
        action: 'financial_diagnostic.failed',
        resource_type: 'diagnostic_session',
        resource_id: sessionId,
        occurred_at: new Date().toISOString(),
        request_id: requestId,
        result: 'failure',
        reason_code: error instanceof Error ? error.constructor.name : 'UnknownError',
